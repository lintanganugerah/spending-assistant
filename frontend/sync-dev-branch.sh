...
################################################################################
# SCRIPT SINKRONISASI BRANCH dev/frontend DENGAN dev
#
# FUNGSI UTAMA:
# Script ini digunakan untuk memperbarui branch `dev/frontend` agar selalu sinkron
# dengan perubahan terbaru dari branch `dev`, sambil tetap menjaga agar folder
# `frontend/` adalah satu-satunya folder yang dipertahankan.
#
# KAPAN HARUS DIGUNAKAN:
# - Saat kamu akan mulai bekerja di `dev/frontend`
...
# 4. Menghapus folder `backend/` dari `dev/frontend`, karena tidak relevan dengan branch ini.
# 5. Membuat commit kosong sebagai penanda bahwa `dev/frontend` sudah disinkronkan.
# 6. Push branch `dev/frontend` ke remote.
#
# CATATAN PENTING:
# - Script ini TIDAK AKAN MENIMPA pekerjaan kamu yang belum di-commit.
# - Namun, jika folder `backend/` masih ada dan berisi kerjaan yang belum kamu merge ke `dev`,
...
echo -e "${GREEN}Branch 'dev/frontend' disinkronkan dan dipush.${NC}"
